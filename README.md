...
웹캠 원본을 방송 화면에 표시하지 않으면서 VRM 등록·캘리브레이션·알파 또는 단색 배경 Spout 송출까지 하나의 앱에서 운영하는 것을 목표로 제작했습니다.

[Windows x64 릴리즈 다운로드](../../releases/latest)

![Virtual Avatar Studio 데모](Docs/Media/virtual-avatar-studio-demo.gif)

*위 데모는 1.0 촬영본으로 파란 크로마 배경을 사용합니다. 1.1.0부터는 알파 기반 배경 제거가 기본 모드입니다.*

## 목차

- [1.1.0 패치 내역](#110-패치-내역)
- [주요 기능](#주요-기능)
- [시스템 구조](#시스템-구조)
- [실행 방법](#실행-방법)
- [VRM 등록과 캘리브레이션](#vrm-등록과-캘리브레이션)
- [OBS 송출](#obs-송출)
- [기술적 해결 내용](#기술적-해결-내용)
  - [추론 파이프라인 최적화](#추론-파이프라인-최적화)
  - [저신뢰 포즈 입력 억제](#저신뢰-포즈-입력-억제)
  - [알파 기반 Spout 출력](#알파-기반-spout-출력)
  - [VRM 안전 교체와 자원 수명주기](#vrm-안전-교체와-자원-수명주기)
  - [성능 패널과 로그](#성능-패널과-로그)
- [개인정보 보호 설계](#개인정보-보호-설계)
- [팔 매핑과 진단](#팔-매핑과-진단)
- [개발 환경과 소스 실행](#개발-환경과-소스-실행)
- [폴더 구성](#폴더-구성)
- [검증 범위](#검증-범위)
- [개발 방식과 역할](#개발-방식과-역할)
- [라이선스와 출처](#라이선스와-출처)
...
![1.0 크로마키와 1.1.0 배경 제거 비교](Docs/Media/background-mode-comparison-1.1.0.png)

- 기존 크로마키 방식을 알파 기반 `배경 제거`와 불투명 `단색 배경` 두 모드로 개편했습니다.
- 신뢰도가 낮거나 갑자기 튀는 어깨·팔 입력을 억제해 아바타 움직임을 안정화했습니다.
- 방송 미리보기에서 드래그 이동·중심 회전·휠 확대/축소와 아바타 구도 초기화를 지원합니다.
- 추적·성능 상태창의 펼침·접힘 위치 복원과 메시지 자동 폭·줄바꿈을 개선했습니다.
- 방송 출력과 앱 종료 과정의 안정성을 개선했습니다.
...
- 신뢰도 기반 좌·우 팔 독립 추적과 이상 입력 억제
- 알파 기반 배경 제거 또는 사용자 지정 RGB 단색 배경
- 방송 미리보기 카메라 이동·중심 회전·확대/축소와 아바타 구도 초기화
- KlakSpout을 이용한 OBS 아바타 RGBA 전용 송출
...
웹캠 프레임은 Sentis 추론 입력으로만 사용합니다. 추론이 끝나면 영상이 아닌 머리·팔 회전값, 표정 수치와 추적 상태를 `TrackingPacket`으로 전달합니다. Main Camera는 아바타를 RGBA RenderTexture에 한 번 렌더링하고 KlakSpout은 해당 텍스처만 OBS로 공유합니다. 운영 화면은 같은 텍스처를 선택 RGB 위에 합성한 뒤 트래킹 UI를 별도로 표시합니다.
...
1. [GitHub Releases](../../releases/latest)에서 1.1.0 Windows ZIP을 받습니다.
...
7. 아바타 페이지에서 `배경 제거` 또는 `단색 배경`을 선택합니다.
...
캘리브레이션할 때는 상체를 카메라 정면에 두고 양팔을 편안하게 내립니다. 손목과 손은 화면에 들어오지 않아도 됩니다. 양쪽 어깨와 팔꿈치의 visibility·presence가 모두 기준을 충족해야 하며, 인식 범위를 벗어나면 최대 5초간 복귀를 기다린 뒤 3초 카운트다운을 다시 시작합니다.
...
4. 앱의 `배경 제거` 모드에서는 Spout 소스의 합성 방식을 `Premultiplied Alpha`로 설정합니다.
5. 앱의 `단색 배경` 모드에서는 합성 방식을 `Opaque`로 설정합니다.
6. 이전 버전에서 추가한 `크로마 키` 필터가 있다면 제거하거나 비활성화합니다.
7. 소스 크기가 캔버스와 다르면 `변환 > 화면에 맞춤`을 실행합니다.
...
Spout에는 Main Camera가 렌더링한 아바타 RGBA와 선택한 송출 배경 모드만 전달됩니다. 카메라·아바타·팔·테스트 페이지, 캘리브레이션 안내, 성능 패널과 익명 추적 뼈대는 트래킹 UI에만 표시됩니다.

![OBS Spout 송출](Docs/Media/obs-spout-output.png)

*1.0의 파란 배경 송출 화면입니다. 1.1.0 배경 제거 모드에서는 크로마 키 없이 premultiplied alpha로 합성합니다.*
...
BlazePose의 33개 랜드마크 출력에는 좌표와 함께 visibility·presence가 포함됩니다. 1.0에서는 좌표만 사용해 화면 밖 또는 낮은 신뢰도의 어깨·팔꿈치도 아바타 팔 회전으로 전달될 수 있었습니다.

1.1.0은 visibility와 presence가 각각 0.5 이상이고 화면 안에 있는 관절만 사용합니다. 먼저 양쪽 어깨의 중심 이동, 어깨선 각도와 폭의 시간적 연속성을 하나의 상체 기준으로 검사합니다. 이 기준이 급변하거나 재인식 중이면 양팔을 함께 최대 0.25초 유지한 뒤 중립으로 복귀시키고, 상체가 0.35초간 안정된 뒤에만 다시 적용합니다. 상체가 안정된 상태에서 한쪽 팔꿈치만 불안정한 경우에는 해당 팔만 독립적으로 처리합니다. 중립 복귀 뒤에도 마지막 정상 방향을 보존하며, 큰 재진입 변화는 연속된 신뢰 입력의 방향이 일치할 때만 평활화해 적용합니다.

### 알파 기반 Spout 출력

Main Camera는 1280×720 RGBA RenderTexture에 아바타를 한 번 렌더링합니다. KlakSpout은 이 텍스처를 `Texture` 캡처와 `Keep Alpha` 설정으로 전송하므로 운영 UI와 웹캠 원본은 포함되지 않습니다.

배경 제거에서는 카메라를 투명 검정으로 지우고, 운영 화면만 선택 RGB 위에 premultiplied alpha로 합성합니다. 단색 배경에서는 선택 RGB를 알파 1의 실제 송출 배경으로 사용합니다. 두 모드 모두 같은 3D 렌더 결과를 사용하며 추론 작업을 추가하지 않습니다.
...
성능 패널은 기본 요약 상태로 접혀 있으며, 제목을 드래그해 이동할 수 있습니다. 요약 길이에 맞춰 화면 범위 안에서 폭이 자동으로 늘어나고 작은 창에서는 두 줄로 전환되므로 상태 문장이 잘리지 않습니다. 펼치면 렌더, 전체 추론 처리·갱신, 얼굴 검출, 포즈 검출, 포즈 랜드마크와 실제 포즈 적용의 순간·EMA 평균을 확인할 수 있습니다.
...
- `Assets/VAS/Broadcast`: RGBA RenderTexture, Spout 출력과 로컬 premultiplied 미리보기
...
- 1.1.0 스크립트·셰이더의 Unity 6000.3.10f1 컴파일과 샘플 씬 로드
- 1.1.0 Windows x64 빌드 성공과 빌드 결과의 `.vrm` 파일 0개
- 배경 제거·단색 배경 모드 저장 및 모드별 카메라 알파 설정의 정적 검사
- 포즈 visibility·presence 기반 캘리브레이션 및 좌·우 팔 독립 신뢰 판정의 정적 검사
- 양쪽 어깨 중심·각도·폭의 연속성 검사와 상체 재인식 대기의 컴파일·빌드 검사
- 추적·성능 상태창의 크기 전환 위치 복원, 자동 폭 확장과 작은 창 두 줄 전환의 Windows Player 실기 검사
- 방송 미리보기의 이동·중심 회전·확대/축소·아바타 구도 초기화, UI 위 입력 차단과 Spout 송출 구도 반영의 Windows Player 실기 검사
- 301바이트 `TrackingPacket`의 구형 포즈 비트 호환과 새 팔 신뢰 비트 판정
- OBS에서 트래킹 UI·웹캠 원본 비노출
- 반복 교체와 정상 종료 시 네이티브 자원 경고·예외 부재

1.1.0 Windows Player의 Spout 캡처 PNG에서 투명 배경과 불투명 아바타 알파를 확인했습니다. 사용자는 양쪽 어깨 오인식 개선과 한쪽 팔꿈치 가림·재진입, 상태창 위치 복원과 전체 요약 표시, 방송 미리보기 카메라 조작·UI 입력 차단·Spout 구도 반영을 검토해 문제가 없음을 확인했습니다. 최종 웹캠·추론 실행에서는 렌더 평균 60 FPS와 추론 갱신 평균 15 FPS를 기록했으며, 정상 종료 시 성능 스냅샷이 한 번 기록되고 웹캠이 해제됐습니다. RenderTexture·ComputeBuffer·CPUTensorData 경고와 예외·오류는 발생하지 않았습니다.

## 개발 방식과 역할

요구사항과 기능 범위, Unity 애플리케이션 구조와 검증 기준은 직접 정의했습니다. OpenAI Codex를 코드 작성과 반복 검증에 활용했으며, 결과물은 Unity 컴파일·Windows Player 빌드와 VRM·웹캠·Spout·OBS 실제 실행으로 확인했습니다.
